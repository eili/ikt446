...
db.factAggrByCountry.aggregate([
    { $group: {
            _id:  { year: "$year" }, 
            amountMNOK: { $sum: "$amount" },
            amountMusd: { $sum: "$amountMusd" },
    ...
    { $project: { year: "$_id.year", amountMNOK: "$amountMNOK", amountMusd: "$amountMusd", kbarrels: "$kbarrels", _id: 0 }},
      { $out: "factAggrByYear"}    
]);  
db.currency.aggregate([    
    { $group: {
            _id:  { year: "$year"}, 
            avgCurrency: { $avg: "$currency" }            
        }
    },
    { $project: { year: "$_id.year", avgCurrency: "$avgCurrency", _id: 0 }},
    { $out: "currencyAvgByYear"}
]);
db.oilprice.aggregate([    
    { $group: {
            _id:  { year: "$year"}, 
            avgValue: { $avg: "$value" }            
        }
    },
    { $project: { year: "$_id.year", avgValue: "$avgValue", _id: 0 }},
    { $out: "oilpriceAvgByYear"}
]);